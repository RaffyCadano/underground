...
export function PricingPlans({
  ...
  return (
    <div className={className}>
      ...
      <ScrollReveal delay={showHeader ? 100 : 0}>
        <div className={`mx-auto flex w-fit flex-col items-center ${showHeader ? 'mt-8' : ''}`}>
          <div className="flex items-center gap-1 rounded-full border border-slate-800 bg-slate-900/80 p-1">
            ...
            >
              Annual
              <span className="rounded-full border border-amber-500/40 bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-bold normal-case tracking-normal text-amber-200">
                Save {PREMIER_ANNUAL_SAVINGS_PERCENT}%
              </span>
            </button>
          </div>
          <p
            className={`mt-3 text-center text-sm ${
              billingPeriod === 'annual' ? 'font-medium text-brand-300' : 'text-slate-500'
            }`}
          >
            Save {PREMIER_ANNUAL_SAVINGS_PERCENT}% by choosing the annual subscription!
          </p>
        </div>
      </ScrollReveal>

      <div
        className={`mx-auto grid max-w-5xl gap-6 ${showHeader ? 'mt-10' : 'mt-8'} ${
          showPremierUpgrade ? 'lg:grid-cols-2 lg:gap-8' : 'max-w-lg'
        }`}
      >
        <ScrollReveal delay={showHeader ? 160 : 80}>
          <PlanCard
            variant="free"
        ...
        {showPremierUpgrade && (
          <ScrollReveal delay={showHeader ? 220 : 140}>
            <PlanCard
              variant="premier"
