...
function emailFrom() {
  return process.env.EMAIL_FROM ?? 'Underground <noreply@underground.local>';
}

export async function sendPasswordResetEmail(to: string, resetUrl: string): Promise<SendResult> {
  const subject = 'Reset your Underground password';
  const text = [
    'You requested a password reset for your Underground account.',
    '',
  ...
  const html = `
    <div style="font-family:system-ui,sans-serif;line-height:1.5;color:#e2e8f0;max-width:480px;margin:0 auto;padding:24px;background:#0f172a;border-radius:12px;border:1px solid #1e293b">
      <p style="margin:0 0 8px;font-size:12px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:#4ade80">Underground</p>
      <h1 style="margin:0 0 16px;font-size:20px;color:#fff">Reset your password</h1>
      <p style="margin:0 0 20px;color:#94a3b8">You requested a password reset for your Underground account. Click the button below to choose a new password.</p>
